...
const setStyleProperty = () => {
  ...
  element.style.setProperty('--count', dataAbout.data.length.toString());
}
...
const addEchart = async (oneDataType: OneDataType) => {
  if (dataAbout.data.length >= 7) {
    ElMessage.warning('最多只能添加7个echarts！');
    return;
  }
...
const initEcharts = () => {
  ...
  // const myChart1: EChartsType = echarts.init(document.getElementById('main1'));
  // const myChart2: EChartsType = echarts.init(document.getElementById('main2'));
  // const myChart3: EChartsType = echarts.init(document.getElementById('main3'));
  // const myChart4: EChartsType = echarts.init(document.getElementById('main4'));

  // const echartsLikageModel_1 = getEchartsLikageModel([{ type: 'line', name: 'Email', seriesData: getSeriesData(count) }] as SeriesOptionType[]);
  // const echartsLikageModel_2 = getEchartsLikageModel([{ type: 'bar', name: 'Union Ads', seriesData: getSeriesData(count) }] as SeriesOptionType[]);
  // const echartsLikageModel_3 = getEchartsLikageModel([{ type: 'line', name: 'Video Ads', seriesData: getSeriesData(count) }] as SeriesOptionType[]);
  // const echartsLikageModel_4 = getEchartsLikageModel([{ type: 'line', name: 'Direct', seriesData: getSeriesData(count) }, { type: 'line', name: 'Search Engine', seriesData: getSeriesData(1000) }] as SeriesOptionType[]);

  // 为echarts对象加载数据 
  // myChart1.setOption(echartsLikageModel_1.getResultOption());
  // myChart2.setOption(echartsLikageModel_2.getResultOption());
  // myChart3.setOption(echartsLikageModel_3.getResultOption());
  // myChart4.setOption(echartsLikageModel_4.getResultOption());
  // //联动配置
...
.echarts-linkage-container {
  ...
  .main-container {
    ...
    .echarts-container {
      // min-height: 200px;
      // max-height: 300px;
      height: calc((100% - var(--gap) * (var(--count) - 1)) / var(--count));
      width: 80%;
