...
export class EchartsLinkageModel {
  ...
  initOptionTemplate = () => {
    ...
    xAxis[0].axisLabel.interval = this.xAxisInterval;
  }

  // 组装option
  setResultOption = () => {
    // 合并参数
    function addOneSeries(option: EChartsOption, param: SeriesOptionType): EChartsOption {
      const defaultParams = {
        type: 'line',
        name: 'demo',
        smooth: true,
    ...
    if (Array.isArray(this.seriesOptionArray)) { // 多个图表
      this.seriesOptionArray.forEach((item: SeriesOptionType) => {
        resOption = addOneSeries(resOption, item);
      });
    } else { // 单个图表
      resOption = addOneSeries(resOption, this.seriesOptionArray[0]);
    }
